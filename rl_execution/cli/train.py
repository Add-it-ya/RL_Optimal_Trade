...
--------
    ...
    rlx-train --agent sac --timesteps 100000 --side buy --tag sac_buy
"""
...
def parse_args():
    ...
    p.add_argument(
        "--agent", required=True, choices=["dqn", "doubledqn", "ppo", "a2c", "sac", "sb3dqn"]
    )
...
def main():
    ...
    exec_config = ExecutionConfig(
        total_inventory=args.inventory,
        horizon=args.horizon,
        side=Side(args.side),
    )

    print(
        f"Training {args.agent} for {args.timesteps:,} steps "
    ...
    factory = make_env_factory(args.agent, exec_config, regime=args.regime, randomized=randomized)
    res = evaluate(
        factory,
        AgentStrategy(agent, args.agent),
