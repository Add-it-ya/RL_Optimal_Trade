...
Examples
--------
    python scripts/run_experiments.py --quick                 # fast smoke pipeline
    python scripts/run_experiments.py --timesteps 120000 --episodes 200
    python scripts/run_experiments.py --no-train               # reuse saved models
"""
...
def parse_args():
    ...
    p.add_argument("--agents", nargs="+",
                   default=["dqn", "doubledqn", "ppo", "a2c", "sac"])
...
def main():
    ...
    for name in args.agents:
        ...
        if args.no_train:
            ...
            agent, reward_logs[disp] = train_agent(
                name, total_timesteps=args.timesteps, exec_config=exec_config,
                randomized=True, seed=args.seed, device=args.device)
            save_agent(agent, name, tag=name,
                       meta={"episode_rewards": reward_logs[disp],
    ...
    results = evaluate_across_regimes(
        strategies, exec_config, regimes=regimes, action_types=action_types,
        n_episodes=args.episodes, base_seed=10_000, progress=False)
    ...
    print(f"Saved figures -> reports/figures/")
    ...
    print("\nNext: python scripts/make_report.py")
    ...
    if rl_names:
        ...
        for b in baselines:
            if b in pivot.columns:
                ...
                print(f"  {best_rl} beats {b:<7} in {wins}/{len(pivot)} regimes "
